...
def prep_lda(train, test, timer, is_all):
    le = preprocessing.LabelEncoder()
    col = "user_id"
    le.fit(list(train[col].values.astype('str')) + list(test[col].values.astype('str')))
    train["user_num"] = le.transform(train[col].values.astype('int'))
    print(train.shape)
    print(test.shape)
    ...
    for col, lda_col in zip(cols, lda_cols):
        train[lda_col] = train[col].fillna(0).astype(int)
        test[lda_col] = test[col].fillna(0).astype(int)
    ...
    test = pd.concat([test, lda_test], axis=1)

    use_col = column_selector.get_lda_col(is_all)
    use_col.append("item_id")
    print(train[use_col].shape)
    print(test[use_col].shape)
    return train[use_col], test[use_col]
