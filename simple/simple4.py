...
PRED_TRAIN = os.path.join(OUTPUT_DIR, "pred_train_old.csv")
PRED_TEST = os.path.join(OUTPUT_DIR, "pred_test_old.csv")
...
OUTPUT_PRED = os.path.join(SUB_DIR, "simple4.csv")
OUTPUT_CV_PRED = os.path.join(SUB_DIR, "simple4_cv.csv")
...
from avito.common import csv_loader, old_columns, pocket_lgb, pocket_timer, pocket_logger, holdout_validator
from avito.fe import additional_fe

logger = pocket_logger.get_my_logger()
timer = pocket_timer.GoldenTimer(logger)
dtypes = csv_loader.get_featured_dtypes()
predict_col = old_columns.get_predict_col()
...
for bagging_index in range(bagging_num):
    ...
    lgb.show_feature_importance(models[0], FEATURE_IMPORTANCE)
