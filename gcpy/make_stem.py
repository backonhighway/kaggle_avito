...
train = pd.read_csv(ORG_TRAIN, nrows=1000*30)
test = pd.read_csv(ORG_TEST, nrows=1000*30)
...
def preprocess(text):
    return [stemmer.stem(word) for word in word_tokenize(text) if word.lower() not in russian_stop and not word.isdigit()]
...
for col in ["title", "description"]:
    ...
    train[col] = train[col].apply(lambda x: ' '.join([stemmer.stem(y) for y in x.split(' ')]))
